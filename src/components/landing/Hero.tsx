...
export function Hero() {
    return (
        <section className="relative overflow-hidden bg-slate-50 dark:bg-neutral-950">
            {/* Background effects */}
            <Particles className="opacity-40" count={40} color="#2563eb" />
            <DotPattern className="opacity-[0.08] [mask-image:radial-gradient(600px_circle_at_center,white,transparent)]" spacing={24} />

            {/* Gradient orbs */}
            <div className="absolute top-20 left-1/4 w-96 h-96 bg-blue-400/20 rounded-full blur-[120px] pointer-events-none" />
            <div className="absolute bottom-20 right-1/4 w-80 h-80 bg-sky-400/15 rounded-full blur-[100px] pointer-events-none" />

            <div className="container mx-auto px-4 pt-20 pb-24 md:pt-28 md:pb-32 relative z-10">
                <div className="max-w-4xl mx-auto text-center">
                    {/* Announcement badge */}
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-100/80 dark:bg-blue-900/30 border border-blue-200/60 dark:border-blue-800/40 mb-8 animate-float-up">
                        <Zap className="w-3.5 h-3.5 text-blue-600" />
                        <span className="text-xs font-semibold text-blue-700 dark:text-blue-300">Nuevo: Punto de Venta integrado con DGII</span>
                    ...
                    <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-8 animate-float-up stagger-2 leading-relaxed" style={{ animationFillMode: "backwards" }}>
                        Facturacion electronica e-CF, contabilidad, POS e inventario.
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8 animate-float-up stagger-3" style={{ animationFillMode: "backwards" }}>
                        <Link href="/register">
                            <ShimmerButton className="text-base px-8 py-4 shadow-xl shadow-blue-500/20">
                                Empieza Gratis — 15 Dias
                                <ArrowRight className="w-4 h-4 ml-2 inline" />
                            </ShimmerButton>
                        </Link>
                        <Link href="#demo">
                            <Button variant="outline" size="lg" className="rounded-full px-6 border-blue-200 hover:border-blue-300 hover:bg-blue-50/50 gap-2 h-[52px]">
                                <Play className="w-4 h-4 text-blue-500" /> Ver Demo
                            </Button>
                        </Link>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2 mb-12 animate-float-up stagger-5" style={{ animationFillMode: "backwards" }}>
                        {BADGES.map(b => (
                            <span key={b} className="px-3 py-1 rounded-full bg-white/80 dark:bg-neutral-900/80 border border-blue-100/50 dark:border-blue-800/30 text-xs font-mono font-bold text-blue-600 shadow-sm backdrop-blur-sm">
                                {b}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-3xl mx-auto animate-float-up stagger-6" style={{ animationFillMode: "backwards" }}>
                        {METRICS.map((m, i) => (
                            <div key={i} className="relative group">
                                <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 to-sky-500/5 rounded-2xl group-hover:from-blue-500/10 group-hover:to-sky-500/10 transition-colors" />
                                <div className="relative p-4 text-center">
                                    <p className="text-2xl md:text-3xl font-black tabular-nums bg-gradient-to-r from-blue-600 to-sky-500 bg-clip-text text-transparent">
                ...
                <div className="mt-20 max-w-5xl mx-auto relative perspective-[2000px]">
                    <div className="absolute -inset-x-4 -inset-y-4 bg-gradient-to-b from-blue-500/10 via-transparent to-transparent rounded-3xl blur-xl pointer-events-none" />

                    {/* Background Settings Window */}
                    <div className="absolute top-0 left-8 right-8 bg-white/40 dark:bg-neutral-900/40 rounded-t-2xl border border-blue-200/30 dark:border-blue-800/20 shadow-xl overflow-hidden transform -translate-y-8 md:-translate-y-12 scale-[0.95] opacity-60 backdrop-blur-sm transition-transform duration-500">
                    ...
                    <div className="relative bg-white dark:bg-neutral-900 rounded-2xl border border-blue-200/50 dark:border-blue-800/30 shadow-2xl shadow-blue-500/20 overflow-hidden transform transition-all duration-700 hover:scale-[1.02] hover:-translate-y-2 hover:shadow-blue-500/30">
                        {/* Browser bar */}
