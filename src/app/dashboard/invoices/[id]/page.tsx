...
export default function InvoiceEditorPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        try {
            ...
            found = emitted.find((i: any) => i.id === routeId);
        } catch { }
        ...
        if (!found) {
            try {
                ...
                found = drafts.find((i: any) => i.id === routeId);
                if (found) isDraft = true;
            } catch { }
        }
        ...
        if (found) {
            // Populate state
            setTipo(found.tipo || 'B01');
            setNcf(found.ecf || found.ncf || 'B0100000001');

            // Reconstruct client structure
            const cName = found.cliente || found.client?.name || '';
            const cRnc = found.rnc || found.client?.rnc || '';
            const cAddress = found.client?.address || '';
            const matchedClient = DEFAULT_CLIENTS.find(c => c.rnc === cRnc) || { id: "custom", rnc: cRnc, name: cName, trade: "", type: "", status: "Activo", email: "", phone: "", address: cAddress };
            setClient(matchedClient);

            setDate(found.date || found.fechaEmision || todayISO);
            setDueDate(found.vencimiento || found.fechaVencimiento || dec31);
            if (found.items && found.items.length > 0) setItems(found.items);
            setNotes(found.notes || '');
            setPaymentTerms(found.paymentTerms || 'Contado');
            if (found.plantilla) setPlantilla(found.plantilla);
        }
    }, [routeId]);
    ...
    const handleSave = (autoPay = false) => {
        const invoiceId = routeId || `INV-${Date.now()}`;
        const invoice = {
            id: invoiceId,
            ecf: ncf,
            tipo,
        ...
        try {
            ...
            const existingIndex = emitted.findIndex((i: any) => i.id === invoice.id);
            if (existingIndex >= 0) {
                emitted[existingIndex] = invoice;
            } else {
                emitted.unshift(invoice);

                // If it was a draft, remove it from drafts
                const drafts = JSON.parse(localStorage.getItem('invoice_drafts') || '[]');
                const draftIndex = drafts.findIndex((d: any) => d.id === invoice.id);
                if (draftIndex >= 0) {
                ...
                const legacy = JSON.parse(localStorage.getItem('invoice_draft') || '{}');
                if (legacy.id === invoice.id) localStorage.removeItem('invoice_draft');
            }
    ...
    const handleSaveDraft = () => {
        ...
        try {
            ...
            // Remove from emitted if user decides to stash back to draft
            const emitted = JSON.parse(localStorage.getItem('invoice_emitted') || '[]');
            const eIdx = emitted.findIndex((e: any) => e.id === draft.id);
            if (eIdx >= 0) {
                emitted.splice(eIdx, 1);
                localStorage.setItem('invoice_emitted', JSON.stringify(emitted));
            }

            toast.success("Borrador guardado", { description: "Puedes continuar editando más tarde en Facturación" });
            router.push('/dashboard/invoices');
        } catch { }
    };
                        ...
                        <div className="grid grid-cols-2 border-b divide-x">
                            ...
                            <div className="p-5 flex flex-col items-end gap-3">
                                <Select value={tipo} onValueChange={v => {
                                    setTipo(v);
                                    setNcf(NCF_SERIES[v] || v + '00000001');
                                }}>
