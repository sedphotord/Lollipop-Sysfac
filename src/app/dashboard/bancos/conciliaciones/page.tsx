...
import { CheckCircleIcon, ArrowDownTrayIcon, ArrowPathIcon, XCircleIcon } from "@heroicons/react/24/outline";
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-500">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button variant="outline"><ArrowDownTrayIcon className="w-4 h-4 mr-2" />Importar OFX</Button>
                    <Button className="bg-primary shadow-lg shadow-primary/20"><ArrowPathIcon className="w-4 h-4 mr-2" />Conciliar</Button>
                </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                    { l: "Movimientos", v: MOVS.length, c: "text-blue-600 bg-blue-500/10", I: ArrowPathIcon },
                    { l: "Conciliados", v: MOVS.filter(m => m.estado === 'conciliado').length, c: "text-emerald-600 bg-emerald-500/10", I: CheckCircleIcon },
                    { l: "Pendientes", v: pendientes, c: pendientes > 0 ? "text-amber-600 bg-amber-500/10" : "text-muted-foreground bg-muted", I: XCircleIcon },
                ].map((k, i) => (
                    ...
                    <div className="border rounded-lg overflow-hidden">
                        <Table>
                            <TableHeader className="bg-muted/50">
                                <TableRow><TableHead>Fecha</TableHead><TableHead>Descripción</TableHead><TableHead className="text-right">Monto Banco</TableHead><TableHead>Estado</TableHead></TableRow>
                            </TableHeader>
                            <TableBody>
                                {MOVS.map((m, i) => (
                                    <TableRow key={i} className="hover:bg-muted/20">
                                        ...
                                        <TableCell>
                                            <Badge variant="outline" className={cn("text-xs gap-1", m.estado === 'conciliado' ? 'text-emerald-600 border-emerald-500/30 bg-emerald-500/10' : 'text-amber-600 border-amber-500/30')}>
                                                {m.estado === 'conciliado' ? <><CheckCircleIcon className="w-3 h-3 inline" />Conciliado</> : <>Pendiente</>}
                                            </Badge>
                                        </TableCell>
                                    </TableRow>
                                ))}
