...
base_template = """import React from 'react';
        ...
        <div className="bg-white p-8 max-w-4xl mx-auto rounded-xl shadow-sm text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>
            <div className="flex justify-between items-start mb-8 border-b pb-6" style={{ borderColor: color.primary }}>
                ...
                <div className="text-right">
                    <h2 className="text-2xl font-black uppercase tracking-widest text-gray-800 mb-2">{doc_title}</h2>
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <p className="flex justify-between gap-8 mb-1"><span className="text-gray-500">No.:</span> <span className="font-bold">{document.number}</span></p>
...
components = [
    ("InvoiceStandard", "FACTURA"),
    ("InvoiceCorporate", "FACTURA"),
    ("InvoiceMinimal", "FACTURA"),
    ("InvoiceModern", "FACTURA"),
    ("InvoiceElegant", "FACTURA"),
    ("QuoteStandard", "COTIZACIÓN"),
    ("QuoteDetailed", "COTIZACIÓN"),
    ("PaymentReceipt", "RECIBO DE PAGO"),
    ("DeliveryNote", "CONDUCE"),
    ("AccountStatement", "ESTADO DE CUENTA"),
]

for name, doc_title in components:
    code = base_template.replace("{name}", name).replace("{doc_title}", doc_title)
    with open(os.path.join(target_dir, f"{name}.tsx"), "w", encoding="utf-8") as f:
        f.write(code)

print("Recreated missing templates.")
